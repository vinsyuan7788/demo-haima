 ...
 * @date 2021/12/5
 */
public class HaimaMultipleClientTest {

    private static final Logger LOG = LoggerFactory.getLogger(HaimaMultipleClientTest.class);
    ...
    public void testConnectClient() throws Exception {
        ...
        clients = new HaimaClient[3];
        for (int i = 0; i < clients.length; i++) {
            clients[i] = new HaimaClient(ClientConfig.CONNECT_STRING, ClientConfig.SESSION_TIMEOUT);
            while (clients[i].getClientConnectionState() != ClientConnectionState.CONNECTED) {
                Thread.sleep(1000);
            };
        }

        for (int i = 0; i < clients.length; i++) {
            long sessionId = clients[i].getSessionId();
